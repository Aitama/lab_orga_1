...
double Taylor5(double y){//Entra el valor del logaritmo al calcular
	...
	double y8 = y4 * y4;
	double y9 = y8 * x;
	double R = y3 * (c1 + y2*(c2 + y2 * c3));
	double L = y9 * (c4 + y2*(c5 + y2*(c6 + y2*c7)));
	double H = c0 * x;
	return 2* (H + R + L);//Se retorna la operacion matematica del logaritmo natural
}
